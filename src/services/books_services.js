...
async function createdBook(isbn, title, autor, publish_year, library_id) {
  ...
  if (library_id !== undefined) {
    ...
    if (!existLibrary) {
      throw new Error(
        "La libreria no existe, puede dejarla desaociado o buscar una que exista"
      );
    }
  }
  ...
  if (
    !equalBook ||
    (equalBook.title === title &&
      equalBook.autor === autor &&
      equalBook.publish_year === publish_year)
  ) {
    ...
    throw new Error(
      "El ISBN esta duplicado con datos de otro autor, año de publicacion y/o titulo diferente, verifique el ISBN"
    );
  }
}
